            ...
            try
            {
                ...
                using (var cnx = ConnectionHelper.GetConnection)
                {
                    ...
                    var query =
                        ...
                        "(" + accionFarmaco + " is null or ( lower(p.\"v_Descripcion2\") like " + accionFarmaco + " or lower(p.\"v_AccionFarmaco\") like " + accionFarmaco + " or lower(p.\"v_PrincipioActivo\") like " + accionFarmaco + " ) ) " +
                        "and pa.i_IdAlmacen = 1 and p.\"i_Eliminado\" = 0 and pa.v_Periodo=  " + DateTime.Now.ToString().Split('/', ' ')[2] + " order by 'Nombre';";
                ...
                using (var dbContext = ConnectionHelper.ContasolContext)
                {
                    ...
                    if (entidad == null)
                    {
                        ...
                        var productoToInsert = new productoDto
                        {
                            ...
                            v_PrincipioActivo =  pobjDto.PrincipioActivo,
                            ...
                            v_Descripcion2 = pobjDto.v_Descripcion2,
                            v_IdMarca =  pobjDto.Laboratorio
                        };
                    ...
                    else
                    {
                        ...
                        prodOriginal.v_PrincipioActivo = pobjDto.PrincipioActivo;
                        ...
                        prodOriginal.d_Peso = 0;
                        prodOriginal.d_PrecioVenta = 0;
                        prodOriginal.i_SolicitarNroLoteIngreso = 1;
                        prodOriginal.i_SolicitarNroLoteSalida = 1;
                        prodOriginal.i_IdTipoProducto = 7;
                        prodOriginal.v_Descripcion2 = pobjDto.v_Descripcion2;
                        prodOriginal.d_PrecioVenta = pobjDto.PrecioVenta;
                        prodOriginal.v_IdMarca = pobjDto.Laboratorio;
        ...
        public List<FacturaCobranza> EmpresaDeudora(string rucEmpresa)
        {
            ...
            using (var cnx = ConnectionHelper.GetConnection)
            {
                ...
                var query = "select " +
                 ...
                 " v.t_FechaVencimiento AS FechaVencimiento, " +
                 " v.v_IdVenta, " +
                "  Sum(d_Total) / CASE WHEN (select count(*) from cobranzadetalle where v_IdVenta = v.v_IdVenta and i_Eliminado=0)= 0 THEN 1 ELSE (select count(*) from cobranzadetalle where v_IdVenta = v.v_IdVenta and i_Eliminado=0)END AS NetoXCobrar,   " +
                "  v.v_SerieDocumento + '-' + v.v_CorrelativoDocumento AS NroComprobante, " +
                 " Sum(cd.d_ImporteSoles) AS TotalPagado, " +
                "  CASE WHEN (Sum(d_Total)/    CASE WHEN (select count(*)  from cobranzadetalle where v_IdVenta = v.v_IdVenta and i_Eliminado=0) = 0 THEN 1 ELSE 	(select count(*) from cobranzadetalle where v_IdVenta = v.v_IdVenta and i_Eliminado=0) END ) -  Sum(cd.d_ImporteSoles)  = 0 THEN 'NO DEBE' ELSE 'DEBE' END AS Condicion  " +
                " from venta v " +
