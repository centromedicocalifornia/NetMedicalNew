...
namespace Sigesoft.Node.Contasol.Integration
{
    public partial class frmAddRecipe : Form
    {
        ...
        public frmAddRecipe(ActionForm actionForm, string idDiagnosticRepository, int recipeId, string protocolId, string serviceId, string categoryName, string LineId, string _receta, string medico, int usuarioIngresa)
        {
            InitializeComponent();
            _recipeId = recipeId;
            ...
            _medico = medico;
            receta = _receta;
            _usuarioIngresa = usuarioIngresa;
        }
            ...
            try
            {
                ...
                switch (_actionForm)
                {
                    ...
                    case ActionForm.Edit:
                        ...
                        txtPrecio.Text = (((_recetaDto.d_SaldoAseguradora == null ? 0 : _recetaDto.d_SaldoAseguradora) + (_recetaDto.d_SaldoPaciente == null ? 0 : _recetaDto.d_SaldoPaciente) ?? 0m) / (_recetaDto.d_Cantidad ?? 0m)).ToString(CultureInfo.InvariantCulture);
                        ...
                        dtpFechaFin.Text = _recetaDto.t_FechaFin.Value.ToString();
        ...
        public static void Solonumeroso(KeyPressEventArgs pE)
        {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                _recetaDto.t_FechaFin = DateTime.Now.AddDays(int.Parse(txtDuracion.Text));
                ...
                _recetaDto.v_ReceipId = receta;
                _recetaDto.v_MedicoTratante = int.Parse(_medico);
                //_recetaDto.i_InsertUserId = _usuarioIngresa;
                _recetaDto.i_IsDeleted = 0;
                ...
                _objRecetaBl.AddUpdateRecipe(ref _pobjOperationResult, _recetaDto, _usuarioIngresa);
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (tienePlan)
                {
                    if (resultplan[0].i_EsCoaseguro == 1)
                    {
                        ...
                        var cadena1 =
                            "select PR.r_MedicineDiscount, OO.v_Name, PR.v_CustomerOrganizationId from Organization OO inner join protocol PR On PR.v_AseguradoraOrganizationId = OO.v_OrganizationId where PR.v_ProtocolId ='" +
                            _protocolId + "'";
                        SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
                        SqlDataReader lector = comando.ExecuteReader();
                        string eps = "";
