...
namespace Sigesoft
{
    ...
    {
        public static void LiquidacionHospitalaria(JoinTicketDetails dataRecetaDetail, List<TiposCuenta> dataFarmaHops, List<ComponentForLiquiCustom> ListServicesAndCost, personDto dataPacient, organizationDto dataOrganization, organizationDto dataAseguradora, List<HospitalizacionCustom> dataHospitalizacion, DatosSeguuro dataSeguro, List<MeidicinasTickets>  listaTipoMedicina,string pathFile)
        {
            ...
            if (dataHospitalizacion != null)
            {
                ...
                //foreach (var item in dataHospitalizacion)
                //{
                //    DateTime inicio = item.d_FechaIngreso.Date;
                //    DateTime fin;

                //    if (item.d_FechaAlta != null || item.d_FechaAlta.ToString() == "00/00/0000 0:0:0")
                //    {
                //        fin = item.d_FechaAlta.Date;
                //    }
                //    else
                //    {
            ...
            {                
                ...
                new PdfPCell(new Phrase("DIAS HAB.", fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_LEFT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 16f, BorderColor = BaseColor.WHITE},
                ...
                if (listaTipoMedicina != null)
                {
                    ...
                    {//para el total de productos usados en las operaciones
                        ...
                        cells = new List<PdfPCell>()
                        {
                            new PdfPCell(new Phrase("", fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 5f, BorderColor = BaseColor.WHITE},
                            new PdfPCell(new Phrase("", fontColumnValue)) {Colspan = 3, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 5f, UseVariableBorders = true, BorderColorTop = BaseColor.BLACK,  BorderColorBottom = BaseColor.WHITE, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE},
                            new PdfPCell(new Phrase("Total : FARMACIA", fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 13f, BorderColor = BaseColor.WHITE},
