...
namespace NetPdf
{
    public class LiquidacionParticularesHosp
    {
        public static void CreateLiquidacionSeguros(JoinTicketDetails dataRecetaDetail, List<TiposCuenta> dataFarmaHops, List<ComponentForLiquiCustom> ListServicesAndCost, personDto dataPacient, organizationDto dataOrganization, organizationDto dataAseguradora, List<HospitalizacionCustom> dataHospitalizacion, DatosSeguuro dataSeguro, List<MeidicinasTickets> listaTipoMedicina, List<RecetasSeguros> listaReceta, string pathFile)
        {
            ...
            {                
                ...
                new PdfPCell(new Phrase(": " + Medico, fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_LEFT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 16f, BorderColor = BaseColor.WHITE},
            ...
            if (ListServicesAndCost != null)
            {
                if (ListServicesAndCost.Count > 0)
                {
                    ...
                    if (listaTipoMedicina != null)
                    {
                            ...
                            cells = new List<PdfPCell>()
                        {
                            ...
                            foreach (var data in listaTipoMedicina)
                            {
                                ...
                                double importe = 0.00;
                                importe = double.Parse(data.Total.ToString());
                                ...
                                cells = new List<PdfPCell>()
                            {
                                ...
                                table = HandlingItextSharp.GenerateTableFromCells(cells, columnWidths, null, fontTitleTable);
                                document.Add(table);
                                totalCoaseguro += double.Parse(data.Total.ToString("N2"));
                                totalFarmacia += decimal.Round(decimal.Parse(importeSinIgv.ToString()), 2);
                            }
                            ...
                            cells = new List<PdfPCell>()
                        {
                            ...
                            new PdfPCell(new Phrase(totalFarmacia.ToString("N2"), fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 13f, BorderColor = BaseColor.WHITE},
                            new PdfPCell(new Phrase("", fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 13f, BorderColor = BaseColor.WHITE},
                            new PdfPCell(new Phrase(totalFarmacia.ToString("N2"), fontColumnValue)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 13f, BorderColor = BaseColor.WHITE},
                    ...
                    double subTotalGeneral = totalConsumoGeneral / float.Parse(igv);
                    double IgvGeneral = totalConsumoGeneral - subTotalGeneral;
                    ...
                    if (Deducible != 0)
                    {
                        TotalParcial = decimal.Parse(totalConsumoGeneral.ToString()) - (precioConsulta * decimal.Parse("1.18"));
                    }
                    else
                    {
                        TotalParcial = 0;
                    ...
                    {
                        _coaseguroGr = decimal.Parse(totalConsumoGeneral.ToString()) * (coaseguro_ / 100);
                    }
                    else
                    {
                        _coaseguroGr = decimal.Parse(TotalParcial.ToString()) * (coaseguro_ / 100);
                    }
                    decimal TotalFinal = decimal.Parse(totalConsumoGeneral.ToString()) - (_coaseguroGr + deducible_);
                    ...
                    cells = new List<PdfPCell>()
                {
                    ...
                    new PdfPCell(new Phrase(totalConsumoGeneral.ToString("N2"), fontColumnValueBold)) {Colspan = 1, HorizontalAlignment = iTextSharp.text.Element.ALIGN_RIGHT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = 13f, BorderColor = BaseColor.WHITE},
