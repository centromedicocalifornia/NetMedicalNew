...
namespace NetPdf
{
    public class ConsentimientoInformadoExamenPRUEBAMOLECULARCovid19_LC
    {
        ...
        public static void CreateConsentimientoInformadoExamenMOLECULARCovid19_LC(ServiceList DataService, string filePDF,
         ...
         List<ServiceComponentList> serviceComponent)
        {
            ...
            ServiceComponentList covid = serviceComponent.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.CONSENTIMIENTO_INFORMADO_PARA_EXAMENES_DE_PRUEBA_MOLECULAR_COVID_19_LC_ID_OC);

            #region INFORMACIÓN GENERAL


            string empresa = "PARTICULAR";
            if (covid != null)
            {
                empresa = covid.ServiceComponentFields.Find(p => p.v_ComponentFieldsId == Sigesoft.Common.Constants.CONSENTIMIENTO_INFORMADO_PARA_EXAMENES_DE_PRUEBA_MOLECULAR_COVID_19_LC_EMPRESA) == null ? "- - -" : covid.ServiceComponentFields.Find(p => p.v_ComponentFieldsId == Sigesoft.Common.Constants.CONSENTIMIENTO_INFORMADO_PARA_EXAMENES_DE_PRUEBA_MOLECULAR_COVID_19_LC_EMPRESA).v_Value1;
            }
            ...
            {    
                ...
                new PdfPCell(new Phrase(empresa == "- - -"?"PARTICULAR":empresa, fontColumnValue)){ Colspan = 8, HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = tamaño_celda, UseVariableBorders=true, BorderColorLeft=BaseColor.WHITE,  BorderColorRight=BaseColor.WHITE,  BorderColorBottom=BaseColor.BLACK, BorderColorTop=BaseColor.WHITE},    
                new PdfPCell(new Phrase("", fontColumnValueBold)){ Colspan = 1 , HorizontalAlignment = iTextSharp.text.Element.ALIGN_LEFT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = tamaño_celda, Border = PdfPCell.NO_BORDER},    
