...
namespace NetPdf
{
    public class PagoMedicoAsitencial_New
    {
        ...
        public static void CreatePagoMedicoAsitencial_New(string filePDF,
            organizationDto infoEmpresaPropietaria, List<PaymentMedical_> Listaliq, string fechaInicio, string fechaFin, SystemUserList medico, int TipoPago_)
        {
            ...
            Listaliq.OrderBy(x => x.d_ServiceDate);

            foreach (var item in Listaliq)
            {
                string tipo = "";
                if (item.tipoAtx == "MEDICO TRATANTE")
                {
                    tipo = "MED TRATANTE"; 
                }
                else
                ...
                cells.Add(cell);
                cell = new PdfPCell(new Phrase(nroreco.ToString() + ". ", fontColumnValueBold)) { HorizontalAlignment = PdfPCell.ALIGN_CENTER, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.WHITE, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                cells.Add(cell);
                ...
                cells.Add(cell);
                cell = new PdfPCell(new Phrase(item.pacientName, fontColumnValue)) { HorizontalAlignment = PdfPCell.ALIGN_LEFT, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.WHITE, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                cells.Add(cell);
                cell = new PdfPCell(new Phrase(tipo + "[" + item.r_PaymentPercentage + " %]", fontColumnValue)) { HorizontalAlignment = PdfPCell.ALIGN_CENTER, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.WHITE, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                cells.Add(cell);
                cell = new PdfPCell(new Phrase(item.v_ComponentName, fontColumnValue)) { HorizontalAlignment = PdfPCell.ALIGN_CENTER, UseVariableBorders = true, BorderColorLeft = BaseColor.WHITE, BorderColorRight = BaseColor.WHITE, BorderColorBottom = BaseColor.WHITE, BorderColorTop = BaseColor.WHITE, MinimumHeight = 15f };
                cells.Add(cell);
                ...
                cells.Add(cell);
                string monto = Convert.ToString(item.r_subTotal);
                decimal numero;
