...
namespace Sigesoft.Node.WinClient.BLL
{
    public class ProtocolBL
    {
        ...
        public List<ProtocolList> GetProtocolPagedAndFiltered_old(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrComponente)
        {
            //mon.IsActive = true;
        ...
        public List<ProtocolList> GetProtocolPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrComponente)
        {
            ...
            try
            {
                ...
                var query = (from A in dbContext.getprotocolpagedandfiltered_sp()
                            select new ProtocolList
                            {
                                v_ProtocolId = A.v_ProtocolId,
                                v_Protocol = A.v_Protocol,
                                v_Organization = A.v_Organization,
                                v_Location = A.v_Location,
                                v_EsoType = A.v_EsoType,
                                v_GroupOccupation = A.v_GroupOccupation,
                                v_OrganizationInvoice = A.v_OrganizationInvoice,
                                v_CostCenter = A.v_CostCenter,
                                v_IntermediaryOrganization = A.v_IntermediaryOrganization,
                                ...
                                v_OrganizationInvoiceId = A.v_OrganizationInvoiceId,
                                ...
                                v_WorkingLocationId = A.v_WorkingLocationId,
                                i_IsActive = A.i_IsActive,
                                v_ComponenteNombre = A.v_ComponenteNombre,
                                AseguradoraId = A.AseguradoraId
                            }).ToList();

                //if (!string.IsNullOrEmpty(pstrFilterExpression))
                //{
                //    query = query.Where(p => pstrFilterExpression).ToList();
                //}
                //if (!string.IsNullOrEmpty(pstrSortExpression))
                //{
                //    query = query.OrderBy(pstrSortExpression).ToList();
                //}
                //if (pintPageIndex.HasValue && pintResultsPerPage.HasValue)
                //{
                //    int intStartRowIndex = pintPageIndex.Value * pintResultsPerPage.Value;
        ...
        public List<ProtocolList> GetProtocolPagedAndFiltereds_1(ref OperationResult pobjOperationResult, string @txtProtocolName, string @v_OrganizationId_1, string @v_LocationId_1,
            int? @i_EsoTypeId, string @v_GroupOccupationId, string @v_WorkingOrganizationId_2, string @v_WorkingLocationId_2, string @v_OrganizationInvoiceId_3, string @v_CustomerLocationId,
            int? @i_ServiceTypeId, int? @i_MasterServiceId, string pstrComponente, int @i_IsActive_)
        {
            //mon.IsActive = true;
            ...
            try
            {
                ...
                var query = (from A in dbContext.getprotocolpagedandfiltered_filt_sp(@txtProtocolName, @v_OrganizationId_1, @v_LocationId_1, @i_EsoTypeId, @v_GroupOccupationId, @v_WorkingOrganizationId_2, @v_WorkingLocationId_2,
                                 @v_OrganizationInvoiceId_3, @v_CustomerLocationId, @i_ServiceTypeId, @i_MasterServiceId, @i_IsActive_)
                             select new ProtocolList
                             {
                                 v_ProtocolId = A.v_ProtocolId,
                                 v_Protocol = A.v_Protocol,
        ...
        public EmpresaDtoCotizacion GetEmpresaByEmpresaId(string idEmpresa)
        {
            //mon.IsActive = true;
            try
            {
                ...
                var objEntity = (from a in dbContext.organization
                                 where a.v_OrganizationId == idEmpresa
                                 select new EmpresaDtoCotizacion
                                 {
                                     EmpresaId = a.v_OrganizationId,
                                     Representante =  a.v_ContacName,
                                     Direccion = a.v_Address
        ...
        public ProtocoloDescuentosDto GetDescuentosProtocolo(string _ServiceId)
        {
            try
            {
                ...
                SqlCommand comando = new SqlCommand(query, connection: conectasam.conectarsigesoft);
                SqlDataReader lector1 = comando.ExecuteReader();
                ProtocoloDescuentosDto _ProtocoloDescuentos = new ProtocoloDescuentosDto();
                while (lector1.Read())
                {
                    _ProtocoloDescuentos.v_ServiceId = lector1.GetValue(0).ToString();
                    _ProtocoloDescuentos.v_ProtocolId = lector1.GetValue(1).ToString();
                    _ProtocoloDescuentos.d_PrecioConsulta = decimal.Parse(lector1.GetValue(2).ToString());
                    _ProtocoloDescuentos.d_DescuentoLaboratorio = decimal.Parse(lector1.GetValue(3).ToString());
                    _ProtocoloDescuentos.d_DescuentoRayosX = decimal.Parse(lector1.GetValue(4).ToString());
                    _ProtocoloDescuentos.d_DescuentoEcografias = decimal.Parse(lector1.GetValue(5).ToString());
                    _ProtocoloDescuentos.d_DescuentoFarmacia = decimal.Parse(lector1.GetValue(6).ToString());
                    _ProtocoloDescuentos.d_DescuentoOdontologia = decimal.Parse(lector1.GetValue(7).ToString());
                    _ProtocoloDescuentos.d_CamaHosp = decimal.Parse(lector1.GetValue(8).ToString());
                    _ProtocoloDescuentos.d_SalaOperaciones = decimal.Parse(lector1.GetValue(9).ToString());
                    _ProtocoloDescuentos.d_PrecioAmbulancia = decimal.Parse(lector1.GetValue(10).ToString());
                }
                lector1.Close();
                conectasam.closesigesoft();
                return _ProtocoloDescuentos;
            }
            catch (Exception ex)
            {
                return null;
        ...
        public CategoriaExamen getCategoriaExamen(string _ComponentName)
        {
            try
            {
                ...
                conectasam.opensigesoft();
                    ...
                    SqlCommand comando = new SqlCommand(query, connection: conectasam.conectarsigesoft);
                    SqlDataReader lector1 = comando.ExecuteReader();
                    CategoriaExamen _CategoriaExamenes= new CategoriaExamen();
                    while (lector1.Read())
                    {
                        _CategoriaExamenes.v_ComponentId = lector1.GetValue(0).ToString();
                        _CategoriaExamenes.v_Name = lector1.GetValue(1).ToString();
                        _CategoriaExamenes.i_CategoryId = int.Parse(lector1.GetValue(2).ToString());
                        _CategoriaExamenes.v_Value1 = lector1.GetValue(3).ToString();
                    }
                    lector1.Close();
                    conectasam.closesigesoft();
                    return _CategoriaExamenes;
            }
            catch (Exception ex)
            {
                return null;
