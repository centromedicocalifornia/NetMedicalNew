...
namespace Sigesoft.Node.WinClient.BLL
{
   public class HistoryBL
   {
       ...
       public List<HistoryList> GetHistoryPagedAndFiltered_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
       {
           //mon.IsActive = true;
       ...
       public List<HistoryList> GetHistoryPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
       {
           ...
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
               var query = from A in dbContext.gethistorypagedandfiltered_sp(pstrPersonId)
                           select new HistoryList
                           {
                               v_HistoryId = A.v_HistoryId,
                               d_StartDate = A.d_StartDate,
                               d_EndDate = A.d_EndDate,
                               v_StartDate = A.d_StartDate == null ? "" : A.d_StartDate.Value.ToString("MMMM / yyyy"),
                               v_EndDate = A.d_EndDate == null ? "" : A.d_EndDate.Value.ToString("MMMM / yyyy"),
                               v_Organization = A.v_Organization,
                               v_TypeActivity = A.v_TypeActivity,
                               i_GeografixcaHeight = A.i_GeografixcaHeight,
                               v_workstation = A.v_workstation,
               ...
               var queryableX = query.AsQueryable();

               if (!string.IsNullOrEmpty(pstrFilterExpression))
               {
                   queryableX = queryableX.Where(pstrFilterExpression);
               }
               if (!string.IsNullOrEmpty(pstrSortExpression))
               {
                   queryableX = queryableX.OrderBy(pstrSortExpression);
               }
            
               if (pintPageIndex.HasValue && pintResultsPerPage.HasValue)
               {
                   int intStartRowIndex = pintPageIndex.Value * pintResultsPerPage.Value;
                   queryableX = queryableX.Skip(intStartRowIndex);
               }
               if (pintResultsPerPage.HasValue)
               {
                   queryableX = queryableX.Take(pintResultsPerPage.Value);
               }

               List<HistoryList> objData = queryableX.ToList();
       ...
       public List<TypeOfEEPList> GetTypeOfEEPPagedAndFiltered_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrHistorytId)
       {
           try
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
       ...
       public List<TypeOfEEPList> GetTypeOfEEPPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrHistorytId)
       {
           ...
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
               var query = from A in dbContext.gettypeofeeppagedandfiltered_sp(pstrHistorytId)
                           select new TypeOfEEPList
                           {
                               v_TypeofEEPId = A.v_TypeofEEPId,
                               v_TypeofEEPName = A.v_TypeofEEPName,
                               i_TypeofEEPId = A.i_TypeofEEPId,
                               r_Percentage = A.r_Percentage,
               ...
               List<TypeOfEEPList> objData = queryableX.ToList();
               pobjOperationResult.Success = 1;
               return objData;
       ...
       #region WorkstationDangers
        public List<WorkstationDangersList> GetWorkstationDangersagedAndFiltered_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrHistorytId)
       {
           try
           {
               SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        public List<WorkstationDangersList> GetWorkstationDangersagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrHistorytId)
        {
            ...
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                var query = (from A in dbContext.getworkstationdangersagedandfiltered_sp(pstrHistorytId)
                            select new WorkstationDangersList
                            {
                                v_WorkstationDangersId = A.v_WorkstationDangersId,
                                v_ParentName = A.v_ParentName,
                                i_DangerId = A.i_DangerId,
                                v_DangerName = A.v_DangerName,
                                i_RecordStatus = A.i_RecordStatus,
                                i_RecordType = A.i_RecordType,
                                v_CreationUser = A.v_CreationUser,
                                v_UpdateUser = A.v_UpdateUser,
                ...
                List<WorkstationDangersList> objData = queryableX.ToList();
                pobjOperationResult.Success = 1;
                return objData;
      ...
      public List<PersonMedicalHistoryList> GetPersonMedicalHistoryPagedAndFilteredByPersonId1_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          //mon.IsActive = true;
          try
          {
      ...
      public List<PersonMedicalHistoryList> GetPersonMedicalHistoryPagedAndFilteredByPersonId1(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = (from A in dbContext.getpersonmedicalhistorypagedandfilteredbypersonid1_sp(pstrPersonId)
                          ...
                          select new PersonMedicalHistoryList
                          {
                              ...
                              i_RecordStatus = A.i_RecordStatus,
                              i_RecordType = A.i_RecordType,
                              v_GroupName = A.v_GroupName,
                              v_TypeDiagnosticName = A.v_TypeDiagnosticName,
                              v_DiagnosticDetail = A.v_DiagnosticDetail,
                              v_CreationUser = A.v_CreationUser,
                              v_UpdateUser = A.v_UpdateUser,
                              d_CreationDate = A.d_CreationDate,
                              d_UpdateDate = A.d_UpdateDate,
                              i_Answer = A.i_Answer.Value
                          }).ToList();
              var queryableX = query.AsQueryable();
              ...
              List<PersonMedicalHistoryList> objData = queryableX.ToList();
              pobjOperationResult.Success = 1;
              return objData;
          ...
          try
          {
              ...
              var query = from A in dbContext.personmedicalhistory
                          ...
                          select new PersonMedicalHistoryList
                          {
                              ...
                              v_Tratamiento = A.v_Tratamiento == null ? "- - -" : A.v_Tratamiento,
                              v_Comentarios = A.v_Comentarios == null ? "- - -" : A.v_Comentarios
                          };
      ...
      public void AddPersonMedicalHistory(ref OperationResult pobjOperationResult, List<personmedicalhistoryDto> pobjpersonmedicalhistoryAdd, List<personmedicalhistoryDto> pobjpersonmedicalhistoryUpdate, List<personmedicalhistoryDto> pobjpersonmedicalhistoryDelete, List<string> ClientSession)
      {
          ...
          try
          {
              ...
              foreach (var item in pobjpersonmedicalhistoryAdd)
              {
                  ...
                  objEntity1.v_Tratamiento = item.v_Tratamiento;
                  objEntity1.v_Comentarios = item.v_Comentarios;
              ...
              if (pobjpersonmedicalhistoryUpdate != null)
              {
                  ...
                  foreach (var item in pobjpersonmedicalhistoryUpdate)
                  {
                      ...
                      objEntitySource1.v_Tratamiento = item.v_Tratamiento;
                      objEntitySource1.v_Comentarios = item.v_Comentarios;
                      objEntitySource1.i_IsDeleted = 0;
      ...
      public List<PersonMedicalHistoryList> GetPersonMedicalHistoryReport_(string pstrPersonId)
      {
          //mon.IsActive = true;
          try
          {
      ...
      public List<PersonMedicalHistoryList> GetPersonMedicalHistoryReport(string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = (from x in dbContext.getpersonmedicalhistoryreport_sp(pstrPersonId)
                           select new PersonMedicalHistoryList
                           {
                               i_Item = int.Parse(x.i_Item.ToString()),
                               v_PersonMedicalHistoryId = x.v_PersonMedicalHistoryId,
                               v_PersonId = x.v_PersonId,
                               v_DiseasesId = x.v_DiseasesId,
                               v_DiseasesName = x.v_DiseasesName,
                               ...
                               v_TypeDiagnosticName = x.v_TypeDiagnosticName,
                               v_DiagnosticDetail = x.v_DiagnosticDetail,
                               i_Answer = x.i_Answer.Value,
                               NombreHospital = x.NombreHospital,
                               v_Complicaciones =  x.v_Complicaciones
                           }).ToList();

              List<PersonMedicalHistoryList> a = new List<PersonMedicalHistoryList>();
              int cont = 1;

              var objData = query.AsEnumerable().GroupBy(g => g.v_DiseasesName).Select(s => s.First());

              foreach (var item in objData)
              {
                  PersonMedicalHistoryList AAA = new PersonMedicalHistoryList();
                  AAA.i_Item += cont;
                  AAA.v_PersonMedicalHistoryId = item.v_PersonMedicalHistoryId;
                  AAA.v_PersonId = item.v_PersonId;
                  AAA.v_DiseasesId = item.v_DiseasesId;
                  AAA.v_DiseasesName = item.v_DiseasesName;
                  AAA.i_TypeDiagnosticId = item.i_TypeDiagnosticId;
                  AAA.d_StartDate = item.d_StartDate;
                  AAA.v_TreatmentSite = item.v_TreatmentSite;
                  AAA.v_GroupName = item.v_GroupName;
                  AAA.v_TypeDiagnosticName = item.v_TypeDiagnosticName;
                  AAA.v_DiagnosticDetail = item.v_DiagnosticDetail;
                  AAA.i_Answer = item.i_Answer;
                  AAA.NombreHospital = item.NombreHospital;
                  AAA.v_Complicaciones = item.v_Complicaciones;
                  a.Add(AAA);
              }
              return a;
      ...
      public List<NoxiousHabitsList> GetNoxiousHabitsPagedAndFilteredByPersonId_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          //mon.IsActive = true;
          try
          {
      ...
      public List<NoxiousHabitsList> GetNoxiousHabitsPagedAndFilteredByPersonId(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = from A in dbContext.getnoxioushabitspagedandfilteredbypersonid_sp(pstrPersonId)
                          select new NoxiousHabitsList
                          {
                              v_NoxiousHabitsId = A.v_NoxiousHabitsId,
                              v_NoxiousHabitsName = A.v_NoxiousHabitsName,
                              v_PersonId = A.v_PersonId,
                              v_Frequency = A.v_Frequency,
                              v_Comment = A.v_Comment,
                              i_TypeHabitsId = A.i_TypeHabitsId.Value,
                              v_TypeHabitsName = A.v_TypeHabitsName,
                              i_RecordStatus = A.i_RecordStatus,
                              i_RecordType = A.i_RecordType,
                              v_CreationUser = A.v_CreationUser,
                              v_UpdateUser = A.v_UpdateUser,
              ...
              List<NoxiousHabitsList> objData = queryableX.ToList();
              pobjOperationResult.Success = 1;
              return objData;
      ...
      public List<NoxiousHabitsList> GetNoxiousHabitsReport_OLD(string pstrPersonId)
      {
          //mon.IsActive = true;
          try
          {
      ...
      public List<NoxiousHabitsList> GetNoxiousHabitsReport(string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = (from x in dbContext.getnoxioushabitsreport_sp(pstrPersonId)
      ...
      public List<FamilyMedicalAntecedentsList> GetFamilyMedicalAntecedentsPagedAndFilteredByPersonId_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          //mon.IsActive = true;
          try
          {
      ...
      public List<FamilyMedicalAntecedentsList> GetFamilyMedicalAntecedentsPagedAndFilteredByPersonId(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = from A in dbContext.getfamilymedicalantecedentspagedandfilteredbypersonid_sp(pstrPersonId)
                          select new FamilyMedicalAntecedentsList
                          {
                              v_FamilyMedicalAntecedentsId = A.v_FamilyMedicalAntecedentsId,
                              v_PersonId = A.v_PersonId,
                              v_DiseasesId = A.v_DiseasesId,
                              v_DiseaseName = A.v_DiseaseName,
                              i_TypeFamilyId = A.i_TypeFamilyId.Value,
                              v_TypeFamilyName = A.v_TypeFamilyName,
                              v_Comment = A.v_Comment,
                              i_RecordStatus = A.i_RecordStatus,
                              i_RecordType = A.i_RecordType,
                              v_CreationUser = A.v_CreationUser,
                              ...
                              i_ParentParameterId = A.i_ParentParameterId
                          };
              var queryableX = query.AsQueryable();
              ...
              List<FamilyMedicalAntecedentsList> objData = queryableX.ToList();
              pobjOperationResult.Success = 1;
              return objData;
      ...
      public List<FamilyMedicalAntecedentsList> GetFamilyMedicalAntecedentsReport(string pstrPersonId)
      {
          ...
          {
              SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
              var query = (from x in dbContext.getfamilymedicalantecedentsreport_sp(pstrPersonId)
