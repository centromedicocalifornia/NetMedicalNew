    ...
    public class HospitalizacionBL
    {
        public List<HospitalizacionList> GetHospitalizacionPagedAndFiltered_OLD(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, DateTime? pdatBeginDate, DateTime? pdatEndDate)
        {
            try
            {
                ...
                var query = from A in dbContext.hospitalizacion
                            ...
                            join E in dbContext.servicecomponent on new { a = D.v_ServiceId, b = "N002-ME000000002" }
                            ...
                            select new HospitalizacionList
                            {
                                ...
                                v_Cie10 = A.v_CIE10Id,
                                v_Diagnostico = A.v_DiseasesName
                            };
                ...
                var hospitalizaciones = (from a in objData
                         select new HospitalizacionList
                         {
                            ...
                            v_Cie10 = a.v_Cie10 == null ? "- - -" : a.v_Cie10,
                            v_Diagnostico = a.v_Diagnostico == null ?"- - -": a.v_Diagnostico
                         }).ToList();
                ...
                foreach (var item in obj)
                {
                    ...
                    hospit.v_Cie10 = item.v_Cie10;
                    hospit.v_Diagnostico = item.v_Diagnostico;
        ...
        public List<HospitalizacionList> GetHospitalizacionPagedAndFiltered(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, DateTime? pdatBeginDate, DateTime? pdatEndDate)
        {
            try
            {
                ...
                var query1 = (from A in dbContext.gethospitalizacionpagedandfiltered_1(pdatBeginDate, pdatEndDate)
                            select new HospitalizacionList
                            {
                                d_FechaAlta = A.d_FechaAlta,
                                d_FechaIngreso = A.d_FechaIngreso,
                                ...
                                v_NroLiquidacion = A.v_NroLiquidacion,
                                d_PagoMedico = A.d_PagoMedico,
                                MedicoPago = A.i_MedicoPago,
                                d_PagoPaciente = A.d_PagoPaciente,
                                PacientePago = A.i_PacientePago,
                                v_MedicoTratante = A.v_MedicoTratante,
                                v_Servicio = A.v_Servicio,
                                v_Cie10 = A.v_Cie10,
                                v_Diagnostico = A.v_Diagnostico,
                                i_Years = A.i_Years.Value,
                                v_ProcedenciaPac = A.v_ProcedenciaPac,
                                v_Comprobantes = A.v_Comprobantes,
                                d_MontoPagado = A.d_MontoPagado.Value,
                                Hosp = A.Hosp,
                                Sop = A.Sop
                            }).ToList();

                List<HospitalizacionList> obj = query1.ToList();
                ...
                foreach (var item in query1)
                {
                    ...
                    hospit.v_Cie10 = item.v_Cie10;
                    hospit.v_Diagnostico = item.v_Diagnostico;
                    hospit.v_ProcedenciaPac = item.v_ProcedenciaPac;
                    hospit.v_Comprobantes = item.v_Comprobantes;
                    hospit.d_MontoPagado = decimal.Round(item.d_MontoPagado,2);
                    hospit.Hosp = item.Hosp;
                    hospit.Sop = item.Sop;
                    ...
                    if (servicios.Count > 0)
                    {
                        ...
                        foreach (var servicio in servicios)
                        {
                            ...
                            oHospitalizacionServiceList.v_DocNumber = servicio.v_DocNumber;
                            oHospitalizacionServiceList.d_FechaAlta = servicio.d_FechaAlta;
                            oHospitalizacionServiceList.Tickets = servicio.Tickets;
                            oHospitalizacionServiceList.Componentes = servicio.Componentes;
                            // acá estoy agregando a las lista
                            HospitalizacionServicios.Add(oHospitalizacionServiceList);
                        }
                        hospit.Servicios = HospitalizacionServicios;
                    }
        ...
        public List<HospitalizacionListNew> GetHospitalizacionPagedAndFilteredNew(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, DateTime? pdatBeginDate, DateTime? pdatEndDate)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                dbContext.CommandTimeout = 100000;
                var query1 = (from A in dbContext.gethospitalizacionpagedandfiltered_1(pdatBeginDate, pdatEndDate)
                              select new HospitalizacionListNew
                              {
                                  d_FechaAlta = A.d_FechaAlta,
                                  d_FechaIngreso = A.d_FechaIngreso,
                                  v_Paciente = A.v_Paciente,
        ...
        public List<HospitalizacionList> GetHospitalizacionPagedAndFilteredEstadisticas(ref OperationResult pobjOperationResult, int? pintPageIndex, int? pintResultsPerPage, string pstrSortExpression, string pstrFilterExpression, DateTime? pdatBeginDate, DateTime? pdatEndDate)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                dbContext.CommandTimeout = 100000;
                var query1 = (from A in dbContext.gethospitalizacionpagedandfilteredestadisticas(pdatBeginDate, pdatEndDate)
                              select new HospitalizacionList
                              {
                                  d_FechaAlta = A.d_FechaAlta,
                                  d_FechaIngreso = A.d_FechaIngreso,
                                  ...
                                  Hosp = A.Hosp,
                                  Sop = A.Sop,
                                  Especialidad1 = A.Especialidad1,
                                  Especialidad2 = A.Especialidad2,
                                  TipoHosp = (A.Hosp == "SI" && A.Sop == "NO") ? "HOSPITALARIO" : ((A.Hosp == "NO" && A.Sop == "SI") || (A.Hosp == "SI" && A.Sop == "SI" && A.v_Servicio == "SALA DE OPERACIONES")) ? "AMBULATORIO" : (A.Hosp == "SI" && A.Sop == "SI" && A.v_Servicio == "HOSPITALIZACIÓN") ? "HOSPITALARIO / SALA DE OPERACIONES": "-",
                              }).ToList();
                ...
                foreach (var item in query1)
                {
                    ...
                    hospit.Hosp = item.Hosp;
                    hospit.Sop = item.Sop;
                    hospit.Especialidad1 = item.Especialidad1;
                    hospit.Especialidad2 = item.Especialidad2;
                    hospit.TipoHosp = item.TipoHosp;
        ...
        public hospitalizacionDto GetHospitalizacionDx(string v_HospitalizacionId)
        {
            ...
            try
            {
                ...
                //var query = from A in dbContext.hospitalizacion
                //            where A.i_IsDeleted == 0 && A.v_HopitalizacionId == v_HospitalizacionId
                ...
                //HospitalizacionList objData = query.;
        ...
        public List<HospitalizacionHabitacionList> BuscarHospitalizacionHabitaciones_OLD(string hospitalizacionId)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        private List<HospitalizacionServiceList> BuscarServiciosHospitalizacion_OLD(string hospitalizacionId)
        {
            try
            {
        ...
        private List<HospitalizacionServiceList> BuscarServiciosHospitalizacion(string hospitalizacionId)
        {
            try
            {
                //acá hace un select a la tabla hospitalizacionService y buscas todos que tengan foranea HospitalizacionId
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                var queryservice = (from A in dbContext.buscarservicioshospitalizacion_1(hospitalizacionId)
                                   select new HospitalizacionServiceList
                                   {
                                       v_HospitalizacionServiceId = A.v_HospitalizacionServiceId,
                                       v_HopitalizacionId = A.v_HopitalizacionId,
                ...
                foreach (var item in queryservice)
                {
                    tickets = new HospitalizacionServiceList();
        ...
        public hospitalizacionDto GetHospitalizacion(ref OperationResult pobjOperationResult, string _HopitalizacionId)
        {
            //mon.IsActive = true;
            try
            {
                ...
                var objEntity = (from a in dbContext.hospitalizacion
                                 where a.v_HopitalizacionId == _HopitalizacionId
                                 select a).FirstOrDefault();
                ...
                foreach (var medico in medicos)
                {
                    ...
                    foreach (var servicio in servicioMedico)
                    {
                        ...
                        if (medico.MedicoTratanteId != null)
                        {
                            ...
                            var pago = pagos.Find(x => x.i_MasterServiceId == servicio.i_MasterServiceId && x.i_MasterServiceTypeId == servicio.i_MasterServiceTypeId);
                            if (pago != null)
                            {
                                oLiquidacionServicios.r_costo = pago.r_costo;
                                oLiquidacionServicios.r_Comision = pago.r_Comision;
                                oLiquidacionServicios.r_Total = pago.r_Total;
                                suma += pago.r_Comision;
                            }
                            else 
                            {
                                oLiquidacionServicios.r_costo = 0;
                                oLiquidacionServicios.r_Comision = 0;
                                oLiquidacionServicios.r_Total = 0;
                                suma += oLiquidacionServicios.r_Comision;
                            }
                    ...
                    oLiquidacionMedicoList.Total = (decimal.Round(suma,2)).ToString();
        ...
        private List<PagosComisiones> ObtenerPagos(string serviceId, int medicoTratanteId, float costoComponente)
        {
             SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
            ...
            var comisionMedico = (from A in dbContext.medico
                ...
                select A).ToList();
            List<PagosComisiones> list = new List<PagosComisiones>();
            foreach (var item in comisionMedico)
            {
                ...
                oPagosComisiones.r_Comision = item.r_Medico == null ? 0 : decimal.Round(((oPagosComisiones.r_costo / (decimal)1.18) * item.r_Medico.Value / 100),2);
                oPagosComisiones.r_Total = item.r_Clinica == null ? 0 : decimal.Round(((oPagosComisiones.r_costo /(decimal)1.18)* item.r_Clinica.Value / 100),2);
                oPagosComisiones.i_MasterServiceId = item.i_MasterServiceId;
                oPagosComisiones.i_MasterServiceTypeId = item.i_MasterServiceTypeId;
                list.Add(oPagosComisiones);
        ...
        public hospitalizacionDto GetHosp(string HospId)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
                ...
                var objEntity = (from a in dbContext.hospitalizacion
                                 where a.v_HopitalizacionId == HospId
                                 select a).FirstOrDefault();
        ...
        public hospitalizacionserviceDto GetHospServ(string HospId)
        {
            try
            {
                SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        //public hospitalizacionDto GetHospitalizacion(ref OperationResult objOperationResult, string _HospitalizacionId)
        //{
        //    try
        //    {
        //        SigesoftEntitiesModel dbContext = new SigesoftEntitiesModel();
        ...
        //        var objEntity = (from a in dbContext.hospitalizacion
        //                         where a.v_HopitalizacionId == _HospitalizacionId
        //                         select a).FirstOrDefault();
