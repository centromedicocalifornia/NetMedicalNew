...
namespace Sigesoft.Node.WinClient.BLL
{
    public class Natclar
    {
        ...
        public List<DiseasesList> DataDiagnosticos(string serviceId)
        {
            SigesoftEntitiesModel ctx = new SigesoftEntitiesModel();
            var listDise = (from dis in ctx.diseases
                join diag in ctx.diagnosticrepository on dis.v_DiseasesId equals diag.v_DiseasesId
                where diag.v_ServiceId == serviceId
                select new DiseasesList
                {
                    v_Name = dis.v_Name,
                    d_CreationDate = diag.d_InsertDate,
        ...
        public List<RecomendationList> DataRecomendaciones(string serviceId)
        {
            SigesoftEntitiesModel ctx = new SigesoftEntitiesModel();
            var listDise = (from rec in ctx.recommendation
                join mas in ctx.masterrecommendationrestricction on rec.v_MasterRecommendationId equals mas
                    .v_MasterRecommendationRestricctionId
                where rec.v_ServiceId == serviceId
                select new RecomendationList
                {
                    v_RecommendationName = mas.v_Name,
                }).ToList();
            return listDise;

        }

        public List<DiseasesList> DataAlergias(string serviceId)
        {
            SigesoftEntitiesModel ctx = new SigesoftEntitiesModel();
            var listDise = (from dis in ctx.diseases
                            join diag in ctx.diagnosticrepository on dis.v_DiseasesId equals diag.v_DiseasesId
                            where diag.v_ServiceId == serviceId && (diag.v_DiseasesId == Constants.ALERGIA_DERIVADOS_LACTEOS || diag.v_DiseasesId == Constants.ALERGIA_FRUTOS_SECOS || diag.v_DiseasesId == Constants.ALERGIA_PELO_ANIMAL)
                            select new DiseasesList
                            {
                                v_Name = dis.v_Name,
                                v_DiseasesId = dis.v_DiseasesId,
                                d_CreationDate = diag.d_InsertDate,
                            }).ToList();
            return listDise;
