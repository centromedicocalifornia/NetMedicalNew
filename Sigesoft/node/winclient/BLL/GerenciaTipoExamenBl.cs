...
namespace Sigesoft.Node.WinClient.BLL
{
    public class GerenciaTipoExamenBl
    {
        ...
        public List<GerenciaTreeTipoExamen> Agrupador(List<GerenciaTipoExamen> data)
        {
            ...
            oGerenciaTreeTipoExamen.Total = decimal.Round((decimal.Parse(data.Sum(s => s.CostoExamen).ToString())),2);
        ...
        public List<GerenciaTreeTipoExamen> Perfiles(List<GerenciaTipoExamen> data, List<GerenciaTreeTipoExamen> list)
        {
            ...
            foreach (var tipoEso in tiposEso)
            {
                ...
                oGerenciaTipoExamen.Total = decimal.Round((decimal.Parse(data.FindAll(p => p.TipoEso == tipoEso.TipoEso).Sum(s => s.CostoExamen).ToString())),2);
                ...
                foreach (var empresa in empresas)
                {
                    ...
                    oemp.Total = decimal.Round((decimal.Parse(data.FindAll(p => p.Empresa == empresa.Empresa && p.TipoEso == tipoEso.TipoEso).Sum(s => s.CostoExamen).ToString())),2);
                    emp.Add(oemp);
                }
