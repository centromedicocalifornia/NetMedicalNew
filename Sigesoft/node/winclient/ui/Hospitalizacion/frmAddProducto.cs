...
namespace Sigesoft.Node.WinClient.UI.Hospitalizacion
{
    public partial class frmAddProducto : Form
    {
        ...
        private void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            var cadena = "select PR.r_MedicineDiscount, PR.r_DiscountExam from protocol PR inner join organization OO on PR.v_CustomerOrganizationId = OO.v_OrganizationId where PR.v_ProtocolId='" + _protocolId + "'";
            ...
            if (result == DialogResult.OK)
            {
                ...
                if (medicamento.i_Insumo == 0)
                {
                    ...
                    txtPrecioInicial.Text = (decimal.Round((decimal)medicamento.PrecioVenta, 2)).ToString();
                ...
                else
                {
                    ...
                    if (GananciaInsumo != "0")
                    {
                        ultraLabel8.Text = "Precio Costo";
                        ultraLabel6.Text = "Aumento %";
                        //Nuevo Precio INSUMO 
                        txtPrecioVenta.Text = (decimal.Round(((medicamento.d_PrecioCosto + (medicamento.d_PrecioCosto * decimal.Parse(GananciaInsumo)) / 100)*decimal.Parse("1.18")), 2)).ToString();
                        txtUnidadProductiva.Text = medicamento.IdLinea;
                        txtPrecioInicial.Text = (decimal.Round((decimal)medicamento.d_PrecioCosto, 2)).ToString();
                        //txtPrecio.Text = (decimal.Round((decimal)medicamento.PrecioVenta, 2)).ToString();
                        txtPPS.Text = (decimal.Round((decimal)medicamento.d_PrecioCosto, 2)).ToString();
                        txtDesctoEPS.Text = GananciaInsumo;
                    }
                    else
                    {
                        //Nuevo Precio con descuento EPS
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (txtCantidad.Text == "0" || txtCantidad.Text == "0.0")
            {
                MessageBox.Show("Error en cantidad, ingrese cantidad correcta", "Error de validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        ...
        void calcular()
        {
            ...
            {
                double Precio = txtPrecioInicial.Text == null ? 0.00 : double.Parse(txtPrecioInicial.Text.ToString());
                double Factor = txtCantidad.Text == null ? 0.00 : double.Parse(txtCantidad.Text.ToString());
                double Descuento = txtDescuento.Text == null ? 0.00 : double.Parse(txtDescuento.Text.ToString());
                if (Descuento == 0.00)
                    txtPrecio.Text = (Precio).ToString();
                else
                    txtPrecio.Text = (Precio - ((Precio* Descuento) / 100)).ToString();
            }
        }
        private void frmAddProducto_Load(object sender, EventArgs e)
        {
            var objDescuentoProtocolo = new ProtocolBL().GetDescuentosProtocolo(_serviceId);

            if (objDescuentoProtocolo != null && objDescuentoProtocolo.d_DescuentoFarmacia != null && objDescuentoProtocolo.d_DescuentoFarmacia != 0)
            {
                txtDescuento.Text = objDescuentoProtocolo.d_DescuentoFarmacia == null || objDescuentoProtocolo.d_DescuentoFarmacia == 0 ? "0.00" : objDescuentoProtocolo.d_DescuentoFarmacia.ToString();
            }   
        ...
        private void txtPrecioInicial_TextChanged(object sender, EventArgs e)
        {
            calcular();
        }
    }
