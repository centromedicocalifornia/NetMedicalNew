...
namespace Sigesoft.Node.WinClient.UI.Hospitalizacion
{
    public partial class frmLiquidacionAseguradora : Form
    {
        ...
        public List<LiquidacionesSeguro> _dataReport = new List<LiquidacionesSeguro>();
        ...
        private List<LiquidacionesSeguro> GetData(string pstrFilterExpression)
        {
            ...
            //var _objData1 = oAseguradoraBL.GetLiquidacionAseguradoraPagedAndFiltered(ref objOperationResult, pstrFilterExpression, pdatBeginDate, pdatEndDate);
            string condicionNombre = txtPacient.Text;
            string condicionempresa = "";
            if (cboEmpresa.SelectedValue.ToString() != "-1")
            {
                condicionempresa = cboEmpresa.Text;

            }
            
            string condicionLiquidacion = textLiquidacion.Text;
            string condicionComprobante = textComprobante.Text;

            var _objData = oAseguradoraBL.Seguro_Tickets(ref objOperationResult, pdatBeginDate, pdatEndDate, condicionNombre, condicionempresa, condicionLiquidacion, condicionComprobante);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (grdData_1.Selected.Rows.Count > 0)
            {
                ...
                ///
                var objSeguro = new PacientBL().Seguro_Information(serviceId);
                ...
                var dataTicket = new ServiceBL().Seguro_Tickets(serviceId);
                ///
                ...
                //LiquidacionHosp.LiquidacionHospitalaria(dataRecetaDetail, dataTicketDetail, ListCostosService, objPacient, objOrganization, objAseguradora, objHospitalizacion, objSeguro,dataTicket, pathFile);
        ...
        private void btnPreLiquidacion_Click(object sender, EventArgs e)
        {
            MergeExPDF _mergeExPDF = new MergeExPDF();
            string serviceId = grdData_1.Selected.Rows[0].Cells["ServicioId"].Value.ToString();
            var objProtocol = new ServiceBL().GetDataProtocolByServiceId(serviceId);
            if (grdData_1.Selected.Rows.Count > 0)
            {
                ...
                var ListCostosService = new ServiceBL().GetServiceAndCost_(serviceId);
                var dataTicket = new ServiceBL().Seguro_Tickets(serviceId);
                var dataReceta = new ServiceBL().Seguro_Receta(serviceId);

                var CostoConsultaDeducible = new ServiciosDetalleBL().PrecioDedudicbleSeguro(serviceId);
                ...
                if (LiqId == "")
                {
                    ...
                    #endregion
                    #region GENERAR LIQUIDACION
                    pathFile = ruta + "LIQUIDACIÓN [RESUMEN]-" + nroliq + ".pdf";
                    LiquidacionSeguros.CreateLiquidacionSeguros(dataRecetaDetail, dataTicketDetail, ListCostosService, objPacient, objOrganization, objAseguradora, objHospitalizacion, objSeguro, dataTicket, dataReceta, pathFile, CostoConsultaDeducible);
                    
                    _mergeExPDF.DestinationFile = rutaBasura + "LIQUIDACIÓN-RESUMEN" + serviceId + ".pdf";
                    List<string> PathList = new List<string>();
                    PathList.Add(pathFile);
                    _mergeExPDF.FilesName = PathList;
                    _mergeExPDF.Execute();
                ...
                else
                {
                    ...
                    #endregion
                    string cadenaRuta = "LIQUIDACIÓN [RESUMEN]-" + nroliq;
                    frmGenerarImprimirLiquidacion frm = new frmGenerarImprimirLiquidacion(cadenaRuta);
                    frm.Text = "Nro de Liq: " + nroliq;
                    frm.ShowDialog();
                    pathFile = ruta + "LIQUIDACIÓN [RESUMEN]-" + nroliq + ".pdf";
                    if (frm.result == false)
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (grdData_1.Selected.Rows.Count > 0)
            {
                ...
                ConexionSigesoft conectasam = new ConexionSigesoft();
                conectasam.opensigesoft();
                var cadena1 = "select SR.v_NroLiquidacion, LQ.v_LiquidacionId from service SR inner join liquidacion LQ on SR.v_NroLiquidacion = LQ.v_NroLiquidacion where SR.v_ServiceId='" + serviceId + "'";
                var comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
                ...
                var servicios = new ServiciosDetalleBL().ServiciosServicioDetalle(serviceId);
                var objHospitalizacion = new PacientBL().GetDataHospitalizacionByServiceId(serviceId);

                var tickets = new ServiciosDetalleBL().Tickets_Detalle(serviceId);
                var recetas = new ServiciosDetalleBL().Receta_Detalle(serviceId);
                ...
                string pathFile = ruta + "LIQUIDACIÓN [DETALLADA]-" + nroliq + ".pdf";


                LiquidacionSegurosDetalle.CreateLiquidacionSeguros(servicios, objPacient, objOrganization, objAseguradora, objHospitalizacion, objSeguro, tickets, recetas, pathFile, CostoConsultaDeducible);
                _mergeExPDF.DestinationFile = rutaBasura + "LIQUIDACIÓN-[DETALLADA]-" + nroliq + ".pdf";
        ...
        private void textLiquidacion_KeyUp(object sender, KeyEventArgs e)
        {

        }

        private void textLiquidacion_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                btnFilter_Click(null, null);
        ...
        private void textComprobante_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Enter)
            {
                btnFilter_Click(null, null);
        ...
        private void btnLiquidaciónGlobal_Click(object sender, EventArgs e)
        {
            MergeExPDF _mergeExPDF = new MergeExPDF();
            string serviceId = grdData_1.Selected.Rows[0].Cells["ServicioId"].Value.ToString();
            var objProtocol = new ServiceBL().GetDataProtocolByServiceId(serviceId);
            if (grdData_1.Selected.Rows.Count > 0)
            {
                ...
                if (LiqId == "")
                {
                    ...
                    LiquidacionSegurosDetalle.CreateLiquidacionSegurosGlobal(servicios, objPacient, objOrganization, objAseguradora, objHospitalizacion, objSeguro, tickets, recetas, pathFile, CostoConsultaDeducible);
                    _mergeExPDF.DestinationFile = rutaBasura + "LIQUIDACIÓN-RESUMEN" + serviceId + ".pdf";
