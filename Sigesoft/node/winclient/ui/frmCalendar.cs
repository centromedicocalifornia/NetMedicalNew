...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmCalendar : Form
    {
        ...
        private void BindGrid()
        {
            ...
            if (grdDataCalendar.DisplayLayout.Bands[0].Columns.Exists("d_ServiceDate"))
            {
                grdDataCalendar.DisplayLayout.Bands[0].Columns["d_ServiceDate"].Hidden = false;
                grdDataCalendar.DisplayLayout.Bands[0].Columns["d_ServiceDate"].Header.Caption = "Fecha de Atención";
                grdDataCalendar.DisplayLayout.Bands[0].Columns["d_ServiceDate"].Header.VisiblePosition = 2;
                grdDataCalendar.DisplayLayout.Bands[0].Columns["d_ServiceDate"].Format = "dd/MM/yyyy hh:mm tt";
                grdDataCalendar.DisplayLayout.Bands[0].Columns["d_ServiceDate"].Width = 120;
            }
...
//                    #endregion
        ...
        private void cambiarFechaDeAtenciónToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (grdDataCalendar.Selected.Rows.Count == 0)
                return;
            ...
            if (grdDataCalendar.Selected.Rows[0].Cells["d_ServiceDate"].Value != null)
            {
                serviceDate = (DateTime)grdDataCalendar.Selected.Rows[0].Cells["d_ServiceDate"].Value;
            }


            frmCalendar_Update_Service_Date frm = new frmCalendar_Update_Service_Date(serviceId, serviceDate);
            frm.ShowDialog();
            btnFilter_Click(sender, e);
        }
        ...
        {
            if (grdDataCalendar.Selected.Rows.Count == 0 || grdDataCalendar.Selected.Rows[0].Cells == null)
            {
                cambiarFechaDeAtenciónToolStripMenuItem.Enabled = false;
                return;
            }

            var serviceDateCell = grdDataCalendar.Selected.Rows[0].Cells["d_ServiceDate"];
            if (serviceDateCell != null && serviceDateCell.Value != null)
            {
                cambiarFechaDeAtenciónToolStripMenuItem.Enabled = true;
            }
            else
            {
                cambiarFechaDeAtenciónToolStripMenuItem.Enabled = false;
