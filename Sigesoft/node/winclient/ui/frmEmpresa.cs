        ...
        private void txtIdentificationNumber_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter && txtIdentificationNumber.TextLength >= 8)
                {
                    ...
                    var urlReniec = "https://dniruc.apisperu.com/api/v1/ruc/" + txtIdentificationNumber.Text.Trim() + "?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJlbWFpbCI6ImFybWlvamFfMTlAaG90bWFpbC5jb20ifQ.EX9vCS6nEsittN2qlCRtBHkFqpEVZxNttuZZU5h7LpU";
                    ...
                    if (DataReniec != null)
                    {
                        ...
                        txtName.Text = desconcat[3].Replace("\"", "").Trim();
                        txtAddress.Text = desconcat[15].Replace("\"", "").Trim() + " " + desconcat[17].Replace("\"", "").Trim() + " - " + desconcat[19].Replace("\"", "").Trim() + " - " + desconcat[21].Replace("\"", "").Trim();

                        txtPhoneNumber.Text = desconcat[7].Replace("\"", "").Trim();
                        txtPhoneNumber.Text = desconcat[7].Replace("[", "").Trim();
                        txtPhoneNumber.Text = desconcat[7].Replace("]", "").Trim();

                        if (actividad.Length >= 2)
                        {
                            _SectorCodigo = txtCiiu.Text = actividad[0].Trim();
                            _SectorName = txtSector.Text = actividad[1].Trim();
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show(@"Nro. de RUC incorrecto", @"Información");
                //throw;
            }
        }
