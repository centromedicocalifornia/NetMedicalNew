...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public partial class frmManagementReports : Form
    {
        ...
        private List<string> _filesNameTtemporal = new List<string>();
        ...
        private void GenerateAtencionIntegral(string pathFile)
        {
            ...
            #endregion
            var listAntecedentes = _serviceBL.ObtenerEsoAntecedentesPorGrupoId(Grupo, GrupoEtario, datosP.v_PersonId);
            ...
            var listEmb = _pacientBL.GetEmbarazos(datosP.v_PersonId);
            ...
            var _listaHabitoNocivos = _historyBL.GetNoxiousHabitsReport(datosP.v_PersonId);
            var _listaPatologicosFamiliares = _historyBL.GetFamilyMedicalAntecedentsReport(datosP.v_PersonId);
            var _listMedicoPersonales = _historyBL.GetPersonMedicalHistoryReport(datosP.v_PersonId);
        ...
        public void reportSolo(List<string> componentIds, string idPacient, string serviceId, string _modo)
        {
            _serviceId = serviceId;
            OperationResult objOperationResult = new OperationResult();
            string ruta;
            ...
            //var x = _filesNameTtemporal.ToList();
            ...
            //if (_modo == "run")
            //{
            //    ruta = Common.Utils.GetApplicationConfigValue("rutaReportesBasura").ToString();
            //    _mergeExPDF.DestinationFile = Application.StartupPath + @"\TempMerge\" + _serviceId + ".pdf";
            //    _mergeExPDF.DestinationFile = ruta + _serviceId + ".pdf"; ;
            //    _mergeExPDF.Execute();
            //    _mergeExPDF.RunFile();
            ...
            //    ruta = Common.Utils.GetApplicationConfigValue("rutaReportes").ToString();
            //    _mergeExPDF.DestinationFile = Application.StartupPath + @"\TempMerge\" + _serviceId + ".pdf";
            //    _mergeExPDF.DestinationFile = ruta + _serviceId + "-" + array[0] + ".pdf";
            //    _mergeExPDF.Execute();
            //}
      ...
      private void chkTodos_CheckedChanged(object sender, EventArgs e)
        ...
        private string ChooseReport(string componentId, string serviceId, string pPacienteId, int pintIdCrystal)
        {
            ...
            switch (componentId)
            {
                ...
                case Constants.INFORME_CERTIFICADO_APTITUD_PARA_EMPRESA:
                    //REPORTE APTITUD PARA EMPRESA
                    var INFORME_CERTIFICADO_APTITUD_EMP = _serviceBL.GetAptitudeCertificateRefact2(ref objOperationResult, _serviceId);

                    if (INFORME_CERTIFICADO_APTITUD_EMP == null)
                    {
                        break;
                    }
                    DataSet ds1_EMP = new DataSet();

                    DataTable dtINFORME_CERTIFICADO_APTITUD_EMP = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(INFORME_CERTIFICADO_APTITUD_EMP);
                    ...
                    ReportDocument rp_EMP;


                    if (TipoServicio_EMP == ((int)TypeESO.Retiro).ToString())
                    {
                        rp_EMP = new Reports.crOccupationalMedicalAptitudeCertificateEMPRESA();
                        rp_EMP.SetDataSource(ds1_EMP);

                        string rutaCertificado = Common.Utils.GetApplicationConfigValue("CertificadoRetiro").ToString();
                        //rp = new Reports.crOccupationalMedicalAptitudeCertificateRetiros();
                        rp_EMP.SetDataSource(ds1_EMP);
                        rp_EMP.ExportOptions.ExportFormatType = ExportFormatType.PortableDocFormat;
                        rp_EMP.ExportOptions.ExportDestinationType = ExportDestinationType.DiskFile;
                        objDiskOpt = new DiskFileDestinationOptions();
                        objDiskOpt.DiskFileName = rutaCertificado + INFORME_CERTIFICADO_APTITUD_EMP[0].v_OrganizationName + "-" + INFORME_CERTIFICADO_APTITUD_EMP[0].v_PersonName + "-" + Constants.INFORME_CERTIFICADO_APTITUD_PARA_EMPRESA + "-" + INFORME_CERTIFICADO_APTITUD_EMP[0].d_ServiceDate.Value.ToString("dd MMMM,  yyyy") + ".pdf";

                        rp_EMP.ExportOptions.DestinationOptions = objDiskOpt;
                        rp_EMP.Export();
                    }
                    else
                    {
                        if (INFORME_CERTIFICADO_APTITUD_EMP[0].i_AptitudeStatusId == (int)AptitudeStatus.AptoObs)
                        {
                            //midificacion por que no sale bien 
                            rp_EMP = new Reports.crCertficadoObservadoEMPRESA();
                            //rp = new Reports.crOccupationalMedicalAptitudeCertificate();
                            rp_EMP.SetDataSource(ds1_EMP);
                    ...
                    rp_EMP.ExportOptions.ExportDestinationType = ExportDestinationType.DiskFile;
                    objDiskOpt = new DiskFileDestinationOptions();
                    objDiskOpt.DiskFileName = _ruta + serviceId + "-" + Constants.INFORME_CERTIFICADO_APTITUD_PARA_EMPRESA + ".pdf";
                    _filesNameToMerge.Add(objDiskOpt.DiskFileName);
                    rp_EMP.ExportOptions.DestinationOptions = objDiskOpt;
                    rp_EMP.Export();
                    rp_EMP.Close();
                ...
                case Constants.INFORME_CERTIFICADO_APTITUD_PARA_TRABAJADOR:
                    //REPORTE APTITUD PARA TRABAJADOR
                    var INFORME_CERTIFICADO_APTITUD_TRAB = _serviceBL.GetAptitudeCertificateRefact(ref objOperationResult, _serviceId);

                    if (INFORME_CERTIFICADO_APTITUD_TRAB == null)
                    {
                        break;
                    }
                    DataSet ds1_TRAB = new DataSet();

                    DataTable dtINFORME_CERTIFICADO_APTITUD_TRAB = Sigesoft.Node.WinClient.BLL.Utils.ConvertToDatatable(INFORME_CERTIFICADO_APTITUD_TRAB);
                    ...
                    if (TipoServicio_TRAB == ((int)TypeESO.Retiro).ToString())
                    {
                        rp_TRAB = new Reports.crOccupationalMedicalAptitudeCertificateTRABAJADOR();
                        rp_TRAB.SetDataSource(ds1_TRAB);

                        string rutaCertificado = Common.Utils.GetApplicationConfigValue("CertificadoRetiro").ToString();
                        //rp = new Reports.crOccupationalMedicalAptitudeCertificateRetiros();
                        rp_TRAB.SetDataSource(ds1_TRAB);
                        rp_TRAB.ExportOptions.ExportFormatType = ExportFormatType.PortableDocFormat;
                        rp_TRAB.ExportOptions.ExportDestinationType = ExportDestinationType.DiskFile;
                        objDiskOpt = new DiskFileDestinationOptions();
                        objDiskOpt.DiskFileName = rutaCertificado + INFORME_CERTIFICADO_APTITUD_TRAB[0].v_OrganizationName + "-" + INFORME_CERTIFICADO_APTITUD_TRAB[0].v_PersonName + "-" + Constants.INFORME_CERTIFICADO_APTITUD_PARA_TRABAJADOR + "-" + INFORME_CERTIFICADO_APTITUD_TRAB[0].d_ServiceDate.Value.ToString("dd MMMM,  yyyy") + ".pdf";

                        rp_TRAB.ExportOptions.DestinationOptions = objDiskOpt;
                        rp_TRAB.Export();
                    }
                    else
                    {
                        if (INFORME_CERTIFICADO_APTITUD_TRAB[0].i_AptitudeStatusId == (int)AptitudeStatus.AptoObs)
                        {
                            //midificacion por que no sale bien 
                            rp_TRAB = new Reports.crCertficadoObservadoTRABAJADOR();
                            //rp = new Reports.crOccupationalMedicalAptitudeCertificate();
                            rp_TRAB.SetDataSource(ds1_TRAB);
                    ...
                    rp_TRAB.ExportOptions.ExportDestinationType = ExportDestinationType.DiskFile;
                    objDiskOpt = new DiskFileDestinationOptions();
                    objDiskOpt.DiskFileName = _ruta + serviceId + "-" + Constants.INFORME_CERTIFICADO_APTITUD_PARA_TRABAJADOR + ".pdf";
                    _filesNameToMerge.Add(objDiskOpt.DiskFileName);
                    rp_TRAB.ExportOptions.DestinationOptions = objDiskOpt;
                    rp_TRAB.Export();
                    rp_TRAB.Close();
        ...
        private void SaveReport(string list)
        {
            var x = list;
            _mergeExPDF.FilesName.Add(x);
            ruta = Common.Utils.GetApplicationConfigValue("rutaReportes").ToString();
            _mergeExPDF.DestinationFile = Application.StartupPath + @"\TempMerge\" + _serviceId + ".pdf";
            _mergeExPDF.DestinationFile = ruta + _serviceId + "-" + array[0] + ".pdf";
            _mergeExPDF.Execute();
            //_filesNameTtemporal.Add(_filesNameToMerge.FirstOrDefault());
            //_filesNameToMerge.Clear();
        }
