...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public partial class frmManagementReports_Async : Form
    {
        ...
        private string rutaConsolidado = Common.Utils.GetApplicationConfigValue("rutaConsolidado").ToString();
        ...
        public frmManagementReports_Async(string serviceId, string EmpresaClienteId, string pacientId, string customerOrganizationName, string dni, string pacientName, string aptitud)
        {
        ...
        private void frmManagementReports_Async_Load(object sender, EventArgs e)
        {
           ...
           List<ServiceComponentList> serviceComponents1 = new List<ServiceComponentList>();
            serviceComponents1 = serviceComponents;
           ...
           var reportsCrystal = ordenReportes.FindAll(p => p.v_ComponentId.Contains("N0"));
           ...
           var serviceComponenteEstado = _serviceBL.ValoresComponente_ManagerReport(_serviceId);
           ...
           var informacionProtocol = _serviceBL.ObtenerInformacionProtocoloCompletoAsService(_serviceId);

           #region SAN MARTIN

           int val = 1;
           if (informacionProtocol.GeneralId == "N002-OO000000734")
           {
               val = 0;
           }
           else 
           {
               if (informacionProtocol.ContraId == "N002-OO000000734")
               {
                   val = 0;
               }
               else
               {
                   if (informacionProtocol.SubContraId == "N002-OO000000734")
                   {
                       val = 0;
                   }
                   else
           ...
           #endregion
           #region MEDICINA
            //PRE-OCUPACIONAL / PERIÓDICO O ANUAL / DE RETIRO / REUBICACIÓN
           if (informacionProtocol.ESOId == 1 || informacionProtocol.ESOId == 2 || informacionProtocol.ESOId == 3 || informacionProtocol.ESOId == 5)
           { //
               if (val == 1)
               {
                   ListOrdenada = ListOrdenada.FindAll(
                      p => p.v_ComponentId.Split('|')[0] != "AUT_LIB_INF_SAN_MARTIN" && p.v_ComponentId.Split('|')[0] != "EV-MED-INF-RESULTADOS");
               }
           }
           else if (informacionProtocol.ESOId == 7)//VISITA
           {
               ListOrdenada = ListOrdenada.FindAll(p => p.v_ComponentId.Split('|')[0] != "CAP" && p.v_ComponentId.Split('|')[0] != "HOC"
                       && p.v_ComponentId.Split('|')[0] != "APT_YANA" &&
                       p.v_ComponentId.Split('|')[0] != "FMT" && p.v_ComponentId.Split('|')[0] != "CONS-INF-AUTORIZAR_EMO" &&
                       p.v_ComponentId.Split('|')[0] != "INF-MED-SO-EX-MED-ANUAL" && p.v_ComponentId.Split('|')[0] != "C_I_EX_NED_COIMO");
               if (val == 1)
               {
                   ListOrdenada = ListOrdenada.FindAll(
                      p => p.v_ComponentId.Split('|')[0] != "AUT_LIB_INF_SAN_MARTIN" && p.v_ComponentId.Split('|')[0] != "EV-MED-INF-RESULTADOS");
           ...
           else if (informacionProtocol.ESOId == 8 || informacionProtocol.ESOId == 17)//ALTURA - ESPIROMETRIA
           {
               ListOrdenada = ListOrdenada.FindAll(p => p.v_ComponentId.Split('|')[0] != "CAP" && p.v_ComponentId.Split('|')[0] != "HOC"
                       && p.v_ComponentId.Split('|')[0] != "APT_YANA"  && p.v_ComponentId.Split('|')[0] != "CI" &&
                       p.v_ComponentId.Split('|')[0] != "INFRES-EVALUACION-MED-AUT" &&
                       p.v_ComponentId.Split('|')[0] != "FMT" && p.v_ComponentId.Split('|')[0] != "CONS-INF-AUTORIZAR_EMO" &&
                       p.v_ComponentId.Split('|')[0] != "INF-MED-SO-EX-MED-ANUAL" && p.v_ComponentId.Split('|')[0] != "DJ-EMPO_YANACOCHA" &&
                       p.v_ComponentId.Split('|')[0] != "C_I_EX_NED_COIMO");
               if (val == 1)
               {
                   ListOrdenada = ListOrdenada.FindAll(
                      p => p.v_ComponentId.Split('|')[0] != "AUT_LIB_INF_SAN_MARTIN" && p.v_ComponentId.Split('|')[0] != "EV-MED-INF-RESULTADOS");
           ...
           else
           {
               ListOrdenada = ListOrdenada.FindAll(p => p.v_ComponentId.Split('|')[0] != "CAP" && p.v_ComponentId.Split('|')[0] != "HOC"
                       && p.v_ComponentId.Split('|')[0] != "APT_YANA" && p.v_ComponentId.Split('|')[0] != "CI" &&
                       p.v_ComponentId.Split('|')[0] != "INFRES-EVALUACION-MED-AUT" && p.v_ComponentId.Split('|')[0] != "DECLARACION-JURADA" &&
                       p.v_ComponentId.Split('|')[0] != "FMT" && p.v_ComponentId.Split('|')[0] != "CONS-INF-AUTORIZAR_EMO" &&
                       p.v_ComponentId.Split('|')[0] != "INF-MED-SO-EX-MED-ANUAL" && p.v_ComponentId.Split('|')[0] != "DJ-EMPO_YANACOCHA" &&
                       p.v_ComponentId.Split('|')[0] != "C_I_EX_NED_COIMO");
               if (val == 1)
           ...
           var historiaClinica = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.ATENCION_INTEGRAL_ID || p.v_ComponentId == Sigesoft.Common.Constants.ATENCION_INTEGRAL_GINECOLOGICA_ID);
           if (historiaClinica == null)
           {
               ListOrdenada = ListOrdenada.FindAll(p => p.v_ComponentId.Split('|')[0] != "ATENCION_INTEGRAL");
           }

           var anexo16 = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.EXAMEN_FISICO_7C_ID);
           if (anexo16 == null)
           {
               ListOrdenada = ListOrdenada.FindAll(
                   p => p.v_ComponentId.Split('|')[0] != "7C-YANACOCHA" && p.v_ComponentId.Split('|')[0] != "7C-SHAHUINDO" && 
                   p.v_ComponentId.Split('|')[0] != "7C-COIMOLACHE" && p.v_ComponentId.Split('|')[0] != "7C-PACASMAYO" &&
                   p.v_ComponentId.Split('|')[0] != "7C-MINSURSANRAFAEL" && p.v_ComponentId.Split('|')[0] != "7C-GOLD_FIELD"  &&
                   p.v_ComponentId.Split('|')[0] != "7C-HUDBAY");
           }

           var rm312 = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.EXAMEN_FISICO_ID);
           if (rm312 == null)
           {
               ListOrdenada = ListOrdenada.FindAll(p => p.v_ComponentId.Split('|')[0] != "312" );
           }
           ...
           if (rx.Count != 0)
           {
               if (rx[0].i_GenderId == (int)Sigesoft.Common.Gender.FEMENINO)
               {
                   ...
                   else if (si_AUTORIZACION == "1")
                   {
                       ListOrdenada = ListOrdenada.FindAll(
                           p =>
                               p.v_ComponentId.Split('|')[0] != "N009-ME000000440");
                   }
               }
               else
               {
           ...
           {
               if (serviceComponents.Count() != 0 && serviceComponents[0].i_GenderId == (int)Sigesoft.Common.Gender.FEMENINO)
               {
                   ListOrdenada = ListOrdenada.FindAll(
                  p =>
                      p.v_ComponentId.Split('|')[0] != "N009-ME000000440");
           ...
           var informeLab = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.INFORME_LABORATORIO_ID);
           if (informeLab == null)
           {
               ListOrdenada = ListOrdenada.FindAll(
                   p => p.v_ComponentId.Split('|')[0] != "ILAB_CLINICO");
           }

           var adicionales_Cielo_Azul = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.ADICIONALES_CIELO_AZUL);
           if (adicionales_Cielo_Azul == null)
           {
               ListOrdenada = ListOrdenada.FindAll(
                   p => p.v_ComponentId.Split('|')[0] != "PAR-COP-CIELO_AZUL" && p.v_ComponentId.Split('|')[0] != "AGLU-KOH-SEC-CIELO_AZUL");
           }
           var adicionales_Lumina = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.ADICIONALES_LUMINA);
           if (adicionales_Lumina == null)
           {
               ListOrdenada = ListOrdenada.FindAll(
                   p => p.v_ComponentId.Split('|')[0] != "PAR-COP-LUMINA" && p.v_ComponentId.Split('|')[0] != "AGLU-KOH-SEC-LUMINA");
           }
               ...
               if (si_lab == "1")
               {
                   ListOrdenada = ListOrdenada.FindAll(
                       p =>
                           ...
                           p.v_ComponentId.Split('|')[0] != "PAR-COP-CIELO_AZUL" && p.v_ComponentId.Split('|')[0] != "AGLU-KOH-SEC-CIELO_AZUL" &&
                           p.v_ComponentId.Split('|')[0] != "PAR-COP-LUMINA" && p.v_ComponentId.Split('|')[0] != "AGLU-KOH-SEC-LUMINA");
               }
               else
               {
                   ListOrdenada = ListOrdenada.FindAll(
           ...
           #region Espiro
           var informe_Espiro = serviceComponents1.Find(p => p.v_ComponentId == Sigesoft.Common.Constants.ESPIROMETRIA_ID);
           if (informe_Espiro == null)
           {
               ListOrdenada = ListOrdenada.FindAll(
                   p => p.v_ComponentId.Split('|')[0] != "INFORME_ESPIRO");
           }
        ...
        private void btnConsolidadoReportes_Click(object sender, EventArgs e)
        {
            ...
            #region OLD
                //bool publicar = false;
                //if (rbConAdjuntos.Checked || rbGenerarPublicar.Checked) { publicar = true; }
            #endregion
            ...
            {
                System.Threading.Tasks.Task.Factory.StartNew(() => _LogicReports.CrearReportesCrystal(_serviceId, _pacientId, Reportes,_customerOrganizationName, _dni, _pacientName, false)).Wait();
                #region OLD
            ...
            MessageBox.Show("Reportes actualizados.", "INFORMACIÓN !", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private void btnNewGeneration_Click(object sender, EventArgs e)
        {
            ...
            //string espiro = Common.Utils.GetApplicationConfigValue("ImgEKGDestino").ToString();ImgESPIROOrigen
            string espiro = Common.Utils.GetApplicationConfigValue("ImgESPIROOrigen").ToString();
            string ekg = Common.Utils.GetApplicationConfigValue("ImgEKGOrigen").ToString();
            string psico = Common.Utils.GetApplicationConfigValue("ImgPSICOOrigen").ToString();
            string interc = Common.Utils.GetApplicationConfigValue("Interconsulta").ToString();
            string psicosensometrico = Common.Utils.GetApplicationConfigValue("ImgPsicosensometricoOrigen").ToString();
            string rxDig = Common.Utils.GetApplicationConfigValue("ImgRxOrigen").ToString();
            string rutaConsolidado = Common.Utils.GetApplicationConfigValue("rutaConsolidado").ToString();
            string reporte = "";
            string[] array = new string[2];
            string[] adjunto = new string[2];
            adjunto[0] = _serviceId;
            adjunto[1] = _pacientName;
            using (new LoadingClass.PleaseWait(this.Location, "Generando..."))
            {
                ...
                {
                    array = report.Split('|');
                    reporte = _ruta + _serviceId + "-" + array[0] + ".pdf";
                    if (File.Exists(reporte))
                    {
                        filesName.Add(reporte);
                    }
                    else
                    {
                        ...
                        System.Threading.Tasks.Task.Factory.StartNew(() => _LogicReports.CrearReportesCrystal(_serviceId, _pacientId, ReportList, _customerOrganizationName, _dni, _pacientName, false)).Wait();
                        //frmManagementReports.reportSolo(ReportList, arrayDatos[1], _serviceId, "exe");
                        filesName.Add(reporte);
                    }
                    
                }
                if (rbSinAdjuntos.Checked == true)
                {
                    MergeAndExcetue(filesName);
                    
                }
                else if (rbConAdjuntos.Checked == true || rbGenerarPublicar.Checked == true)
                {
                    string nomenclatura = GetNomenclatura(_serviceId);
                    array = nomenclatura.Split('|');
                    
                    ObtenerAdjuntos(espiro, array);
                    ObtenerAdjuntos(ekg, array);
                    ObtenerAdjuntos(psico, array);
                    ObtenerAdjuntos(rxDig, array);
                    ObtenerAdjuntos(psicosensometrico, array);
                    ObtenerAdjuntos(interc, adjunto);
                ...
                else if (rbSinAdjuntosWeb.Checked == true)
                {
                    string nomenclatura = GetNomenclatura(_serviceId);
                    array = nomenclatura.Split('|');
        ...
        private void ObtenerAdjuntos(string ruta, string[] array)
        {
            DirectoryInfo di = new DirectoryInfo(ruta);
            var Files = di.GetFiles();
            foreach (var item in Files)
            {
                if (item.Name.Contains(array[0]) && item.Name.Contains(array[1]) && item.Name.Contains(".pdf"))
                {
                    //bool result = VerificarExiste(filesName, array);
                    //if (!result)
                    //{
                        filesName.Add(ruta + item.Name);
                    //}
                }
            }
        }

        private bool VerificarExiste(List<string> filesName, string[] array)
        {
            bool result = false;
            foreach (var fi in filesName)
            {
                if (fi.Contains(array[0]) && fi.Contains(array[1]))
                {
                    result = true;
                    break;
                }
        ...
        private void MergeAndExcetue(List<string> files)
        {
            _mergeExPDF.FilesName = files;
            _mergeExPDF.DestinationFile = _ruta + _serviceId + ".pdf";
            _mergeExPDF.Execute();
            _mergeExPDF.RunFile();
            var oService = _serviceBL.GetServiceShort(_serviceId);
            _mergeExPDF.FilesName = files;
            _mergeExPDF.DestinationFile = Application.StartupPath + @"\TempMerge\" + oService.Empresa + " - " + oService.Paciente + " - " + oService.FechaServicio.Value.ToString("dd MMMM,  yyyy") + " - " + oService.Servicio + ".pdf";
            if (oService.Empresa != oService.Contract)
            {
                _mergeExPDF.DestinationFile = rutaConsolidado + oService.Empresa + " - Contrata (" + oService.Contract + ") - " + oService.Paciente + " - " + oService.FechaServicio.Value.ToString("dd MMMM,  yyyy") + " - " + oService.Servicio + ".pdf";
                _mergeExPDF.Execute();
            }
            else if (oService.Empresa == oService.Contract)
            {
                _mergeExPDF.DestinationFile = rutaConsolidado + oService.Empresa + " - " + oService.Paciente + " - " + oService.FechaServicio.Value.ToString("dd MMMM,  yyyy") + " - " + oService.Servicio + ".pdf";
                _mergeExPDF.Execute();
            }         
        }

        private string GetNomenclatura(string _serviceId)
        {
            ConexionSigesoft conectasam = new ConexionSigesoft();
            conectasam.opensigesoft();
            //var cadena1 = @"CONVERT(VARCHAR(2),(d_ServiceDate),103), CONVERT(VARCHAR(2),(d_ServiceDate),101), YEAR(d_ServiceDate), v_ProtocolId from service where v_ServiceId='" + _serviceId + "'";
            var cadena1 = @"SELECT FORMAT(DAY(d_ServiceDate), '00'), FORMAT(MONTH(d_ServiceDate),'00'), YEAR(d_ServiceDate), v_ProtocolId from service where v_ServiceId='" + _serviceId + "'";
            SqlCommand comando = new SqlCommand(cadena1, connection: conectasam.conectarsigesoft);
            SqlDataReader lector1 = comando.ExecuteReader();
            string nom ="";
            while (lector1.Read())
            {
                nom = _dni+"-" + lector1.GetValue(0).ToString() + lector1.GetValue(1).ToString() + lector1.GetValue(2).ToString() +"|"+ lector1.GetValue(3).ToString();
            }
            lector1.Close();
        ...
        {
            List<string> report;
            string cadena = "CI|CONSINF-HUDBAY|DECLARACION_CI|DECLARACION-JURADA|DJ-EMB-RX-SL|DJ-EMO_SECURITAS|REGISTRO_ENTREGA_INFORME_BUENAVENTURA|DJ-EMPO_YANACOCHA|EV-MED-INF-RESULTADOS|EXO-LAB-SL|EXO-RX-SL|AUT_LIB_INF_SAN_MARTIN|AUT_REAL_EX_MED_LIBER_INF|C_I_EX_NED_COIMO";
        ...
        private void btnBuscarReport_Click(object sender, EventArgs e)
        {
            List<string> reports = new List<string>();
            foreach (var item in chklConsolidadoReportes.Items)
            {
                string[] report = ((Sigesoft.Node.WinClient.BE.ServiceComponentList) (item)).v_ComponentId.Split('|');
                string path = _ruta + _serviceId + "-" + report[0] + ".pdf";
                if (!File.Exists(path))
                {
                    reports.Add(report[0]);
                }
            }
            System.Threading.Tasks.Task.Factory.StartNew(() => _LogicReports.CrearReportesCrystal(_serviceId, _pacientId, reports, _customerOrganizationName, _dni, _pacientName, false)).Wait();
        }
    }
}
