...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public class LogicReports
    {
        ...
        private string ChooseReport(string componentId, string serviceId, string pPacienteId, int pintIdCrystal)
        {
            ...
            switch (componentId)
            {
                ...
                case Constants.LABORATORIO_CONSENTIMIENTO_INFORMADO_VIH_ID:
                    GenerateConsentimientoInformadoVIH(string.Format("{0}.pdf", Path.Combine(_ruta, _serviceId + "-" + Constants.LABORATORIO_CONSENTIMIENTO_INFORMADO_VIH_ID)));
                    _filesNameToMerge.Add(string.Format("{0}.pdf", Path.Combine(_ruta, _serviceId + "-" + componentId)));
                    break;
        ...
        private void GenerateConsentimientoInformadoVIH(string pathFile)
        {
            //var _DataService = _serviceBL.GetInformacion_OtrosExamenes(_serviceId);
            var datosP = _pacientBL.DevolverDatosPaciente(_serviceId);
            var MedicalCenter = _serviceBL.GetInfoMedicalCenter();
            ...
            ConsensimientoInformadoVIH.CreateConsentimientoInformadoCIH(pathFile, datosP, MedicalCenter, filiationData, serviceComponents);
        }
