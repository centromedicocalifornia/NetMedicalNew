...
namespace Sigesoft.Node.WinClient.UI
{
    public partial class frmDiseases : Form
    {
        ...
        List<DiseasesList> DiseasesListGlobal = new List<DiseasesList>();
        List<DiseasesList> DiseasesListGlobal_Temp = new List<DiseasesList>();
        ...
        private void grdData_Click(object sender, EventArgs e)
        {
            try
            {
                var DiseaseId = grdData.Selected.Rows[0].Cells["v_DiseasesId"].Value.ToString();
                if (DiseaseId != "")
                {
                    btnUpdateandSelect.Enabled = false;
                    label3.Text = "DX YA ESTÁ EN USO POR OTROS PACIENTES, SI DESEA MODIFICAR BUSQUE AQUEL QUE NO TIENE CÓDIGO INTERNO";
                }
                else
                {
                    btnUpdateandSelect.Enabled = true;
                    label3.Text = "DX CON PERMISOS PARA MODIFICAR";
                }
        ...
        private void btnUpdateandSelect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (grdData.Selected.Rows[0].Cells[0].Value != "")
                    {
        ...
        private void txtDiseasesFilter_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                if (rbDescription.Checked == true)
                {
                    ...
                    {
                        DiseasesListGlobal_Temp = new List<DiseasesList>(DiseasesListGlobal.Where(p => p.v_Name.Contains(txtDiseasesFilter.Text.ToUpper())));

                        grdData.DataSource = DiseasesListGlobal_Temp;

                        if (DiseasesListGlobal_Temp != null && DiseasesListGlobal_Temp.Count() != 0)
                        {
                            lblRecordCount.Text = string.Format("Se encontraron {0} registros.", DiseasesListGlobal_Temp.Count());
                    ...
                    {
                        grdData.DataSource = DiseasesListGlobal;

                        if (DiseasesListGlobal != null)
                        {
                            lblRecordCount.Text = string.Format("Se encontraron {0} registros.", DiseasesListGlobal.Count());
                ...
                else
                {
                    ...
                    if (txtDiseasesFilter.Text != string.Empty)
                    {
                        DiseasesListGlobal_Temp = new List<DiseasesList>(DiseasesListGlobal.Where(p => p.v_CIE10Id.Contains(txtDiseasesFilter.Text.ToUpper())));
            ...
            {
                //txtDiseasesFilter_KeyUp(sender, e);
                throw;
            }
