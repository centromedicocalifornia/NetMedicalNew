    ...
    public partial class frmExternalUserEdit : Form
    {
        ...
        private void LoadCheckedListForUpdate()
        {
            ...
            for (int i = 0; i < chklNotificaciones.Items.Count; i++)
            {
                ...
                {
                    // Buscar Id en mi lista temp
                    var findResult = _notificationUserExternal == null ? null : _notificationUserExternal.Find(p => p.Id == obj.Id);
                    // Si se encontro coincidencia quiere decir que se ha removido el permiso
                    if (findResult != null)
                    {
        ...
        private void LoadData()
        {
            ...
            else if (_mode == "Edit")
            {
                ...
                dtpBirthdate.Value = objPerson.d_Birthdate == null ? DateTime.Now : objPerson.d_Birthdate.Value;
